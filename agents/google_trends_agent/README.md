...
2.  **Installation**
    ...
    cd adk-samples/python/agents/google_trends_agent
...
3.  **Configuration**
    ...
    - Set up your Google Cloud credentials. You can set these in your shell or create a `.env` file in the agent's root directory (`google_trends_agent/`).
...
1.  **To run the agent from the CLI:**
    ...
    Make sure your working directory is `adk-samples/python/agents/google_trends_agent/google_trends_agent`, then run:
...
2.  **To run the agent from the ADK web UI:**
    ...
    adk web .
    ```
    Then select `google_trends_agent` from the dropdown menu.
...
**Create the project from this sample** (replace `my-google_trends_agent` with your project name):

```bash
agents-cli create my-google_trends_agent -a adk@google-trends
```
