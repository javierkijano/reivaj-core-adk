...
![bidi_demo-screen](assets/bidi_demo-screen.png)
...
**Create the project from this sample** (replace `my-bidi_demo` with your project name):

```bash
agents-cli create my-bidi_demo -a adk@bidi_demo
```
...
From the `src/bidi_demo` directory, first change to the `app` subdirectory:
...
runner = Runner(app_name="bidi_demo", agent=agent, session_service=session_service)
```
...
From the `src/bidi_demo` directory, deploy with `gcloud run deploy`:

**Vertex AI Live API mode** (`GOOGLE_GENAI_USE_VERTEXAI=TRUE`):

```bash
gcloud run deploy bidi_demo \
  --source . \
...
```bash
gcloud run services add-iam-policy-binding bidi_demo \
  --project "${GOOGLE_CLOUD_PROJECT}" \
  --region "${GOOGLE_CLOUD_LOCATION}" \
  --member=allUsers \
...
```bash
gcloud run services describe bidi_demo \
  --project "${GOOGLE_CLOUD_PROJECT}" \
  --region "${GOOGLE_CLOUD_LOCATION}" \
...
Agent Engine hosts **agents**, not web applications. It does not expose a public URL or serve static files — access is via authenticated Vertex AI SDK WebSocket connections only. This makes it well suited for backend agent hosting, but not for serving the full bidi_demo webapp directly.
