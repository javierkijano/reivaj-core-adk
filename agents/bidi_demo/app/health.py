"""Standalone health check proxy for bidi_demo Cloud Monitoring uptime checks.

Connects to the bidi_demo WebSocket endpoint as a client and verifies
the model responds to a text query. Designed to run as a separate Cloud Run
service that monitors the main bidi_demo service end-to-end:

    Cloud Monitoring -> GET /health/live -> this app -> WS -> bidi_demo -> Live API

Endpoints:
    GET /health      - Basic reachability check (returns {"status": "ok"})
    GET /health/live - End-to-end model check via bidi_demo WebSocket

Environment variables:
    BIDI_DEMO_URL: WebSocket base URL of the bidi_demo service
                   (default: ws://localhost:8000)

Local testing:
    # Start bidi_demo on port 8000, then:
...
Deploy to Cloud Run:
    gcloud run deploy bidi_demo-health \
      --source . \
      ...
      --set-env-vars BIDI_DEMO_URL=wss://bidi_demo-xxx.us-east1.run.app

Create uptime check:
    gcloud monitoring uptime create "bidi_demo /health/live" \
      --resource-type=uptime-url \
      --resource-labels=host=bidi_demo-health-xxx.us-east1.run.app,project_id=PROJECT \
      --protocol=https \
      --path=/health/live \
      --port=443 \
      --request-method=get \
...
async def health_live():
    """End-to-end health check that connects to bidi_demo as a WebSocket client."""
