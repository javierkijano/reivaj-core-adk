...
This agent first defines whether the user input is actually a decomposition
problem, then transforms complex goals into validated ADK 2.0 graph-based
workflow implementation plans. It does not execute real-world actions yet. V0
focuses on producing structured, verifiable planning data:
...
- repaired `TaskGraph`
- `ExecutionSchedule`
- extensive markdown implementation plan
- decomposition trace
...
-> ProblemDefinitionNormalizer
-> explicit root gate
   -> stop for greetings, simple requests or unresolved approval checkpoints
-> GoalInterpreter
-> StateAbstractor
-> CandidateTaskPlanner
-> Repair loop
   -> Sequential quality checkers
      -> CoverageChecker
      -> GranularityChecker
      -> DependencyChecker
...
The generated implementation plan is aligned with the
`reivaj-adk-2.0-development` guidelines: graph-based `Workflow` by default,
explicit edges/routes, `JoinNode` for convergence, `Event(output=...)` or
`Event(state=...)` for internal data, deterministic tests, and no collaborative
agents or dynamic workflows unless explicitly opted in or clearly necessary.
