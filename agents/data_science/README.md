...
### Architecture
![Data Science Architecture](data_science-architecture.svg)
...
1.  **Clone the Repository:**
    ...
    cd adk-samples/python/agents/data_science
    ```
...
1.  **Install Dependencies with uv:**
    ...
    inside `adk-samples/python/agents/data_science`. If you already have a virtual
...
1.  **BQML Setup:**
    ...
    `data_science/data_science/utils/reference_guide_RAG.py` by running the
...
`data_science/data_science/utils/data/` directory. To load this dataset into
BigQuery, make sure you are still in the working directory
(`agents/data_science`). Then run the following commands:
```bash
...
1. Run this command from the `data_science/flights_dataset` directory to
...
1. Run this command to create a new BigQuery dataset:
    ...
    ```
1. Run these commands from the `data_science/flights_dataset` directory to load the
data into BigQuery:
...
**Create the project from this sample** (replace `my-data_science` with your project name):

```bash
agents-cli create my-data_science -a adk@data_science
```
...
the following uv command from the `agents/data_science` directory to install them:
```bash
uv sync --dev
```
...
Next, you need to create a `.whl` file for your agent. From the `data_science`
...
*   **Extension:** Extend the multi-agent system with your own AgentTools or
    ...
    root agent inside `agents/data_science/data_science/agent.py`.
...
```bash
gcloud run deploy data_science-agent \
  --source . \
  --port 8080 \
  --memory 2G \
  --project $PROJECT_ID \
...
```bash
Service [data_science-agent] revision [data_science-agent-00001-aaa] has been deployed and is serving 100 percent of traffic.
```
