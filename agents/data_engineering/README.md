...
**Create the project from this sample** (replace `my-data_engineering-agent` with your project name):

```bash
agents-cli create my-data_engineering-agent -a adk@data_engineering
```

The Google Agents CLI will prompt you to select deployment options and set up your Google Cloud project.

From your newly created project directory (e.g. `my-data_engineering-agent`), configure your environment and run:

```bash
cd my-data_engineering-agent
...
1. Clone the repository:
    ...
    cd adk-samples/python/agents/data_engineering
    ```
    For the rest of this tutorial **ensure you remain in the `python/agents/data_engineering` directory**.
