...
**Create the project from this sample** (replace `my-short_movie_agents` with your project name):

```bash
agents-cli create my-short_movie_agents -a adk@short_movie_agents
```

The Google Agents CLI will prompt you to select deployment options and set up your Google Cloud project.

From your newly created project directory (e.g. `my-short_movie_agents`), run:

```bash
cd my-short_movie_agents
uv sync --dev
uv run adk run app
```
...
1. **Clone and enter the sample directory:**
   ...
   cd adk-samples/python/agents/short_movie_agents
   ```

   Stay in `python/agents/short_movie_agents` for the steps below.
