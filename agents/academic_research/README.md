...
<img src="academic_research.svg" alt="academic researcher" width="800"/>
...
**Create the project from this sample** (replace `my-academic_research` with your project name):

```bash
agents-cli create my-academic_research -a adk@academic_research
cd my-academic_research
```
...
cd adk-samples/python/agents/academic_research
uv sync --group dev
```
...
Then the tests and evaluation can be run from the `academic_research` directory using
the `pytest` module:
